...
mod tests {
    ...
    fn visit_borrowed_bytes() {
        let visitor: TryFromStrVisitor<DomainRef> =
            TryFromStrVisitor::new("a borrowed domain string");
        let deserializer: BorrowedBytesDeserializer<ValueError> =
            BorrowedBytesDeserializer::new(b"localhost");
        assert_eq!(
        ...
        let visitor: TryFromStrVisitor<DomainRef> =
            TryFromStrVisitor::new("a borrowed domain string");
        let deserializer: BorrowedBytesDeserializer<ValueError> =
            BorrowedBytesDeserializer::new(b"\xFF");
