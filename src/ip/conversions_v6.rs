...
impl IPv6Address {
    ...
    /// to accept only mapped addresses.
...
impl TryFrom<IPAddress> for IPv6Address {
...
mod tests {
    ...
    fn v6_to_host() {
        ...
        let expected: Host = Host::IPAddress(IPAddress::V6(IPv6Address::LOCALHOST));
        assert_eq!(result, expected);

        let result: HostRef = ip.to_host_ref();
        let expected: HostRef = HostRef::IPAddress(IPAddress::V6(IPv6Address::LOCALHOST));
        assert_eq!(result, expected);
    }
    ...
    fn v6_try_from() {
        ...
        assert_eq!(IPv6Address::try_from(ip), Err(ip));
    }
}
